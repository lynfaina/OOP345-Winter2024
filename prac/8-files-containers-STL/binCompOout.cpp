...
// instead of converting it and using the operator overload from the header file which are these:
// (std::ostream& operator<<(std::ostream& os, const Employee& E);
// (std::istream& operator>>(std::istream& ifs, Employee& E);)
// completely ignore those and just dump the class into the file 
// employee file now is in the binary file
// all the ASCII are visible but the rest of the properties that are
// binary are not visible
// its literally dump of the memory of the classes one by one into
// the file, obviously the size of the file is much bigger
// because it writes the entire 52 characters for name
